...
export function AccountsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-800">Mis Cuentas</h1>
        <p className="text-slate-500 mt-1">Consulta el saldo y estado de tus cuentas.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {accounts.map((account) => (
            <div key={account.accountId} className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow relative overflow-hidden">
              <div className="absolute top-0 right-0 w-24 h-24 bg-green-50 rounded-bl-full -z-10 opacity-50"></div>
              
              <div className="flex justify-between items-start mb-4">
                <div>
                  <p className="text-sm font-semibold text-slate-400 uppercase tracking-wider">CUENTA DE {account.type || 'AHORRO'}</p>
                  <p className="text-xl font-bold text-slate-800 tracking-tight mt-1">{account.accountNumber}</p>
                </div>
                <div className={`px-2.5 py-1 rounded-full text-xs font-bold ${
                  account.status === 'ACTIVA' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                }`}>
                  {account.status}
                </div>
              </div>
